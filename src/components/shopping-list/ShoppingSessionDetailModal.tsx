...
} from "./ShoppingSessionCloseModal";
import type { ShoppingListItem, ShoppingSessionDetail } from "@/types/models";
...
export function ShoppingSessionDetailModal({
  houseId,
  sessionId,
  pendingItems,
  onClose,
  ...
  return (
    <>
      <Modal
        open
        onClose={onClose}
        title={session ? `Sesión · ${session.name}` : "Sesión de compra"}
      >
              ...
              {typeof session.expense === "object" && session.expense && (
                <div className="rounded-lg border border-black/[.08] px-4 py-3 text-sm dark:border-white/[.145]">
                  ...
                  <p className="font-medium text-black dark:text-zinc-50">
                    {session.expense.name} ·{" "}
                    {formatCurrency(session.expense.amount)}
                  </p>
                </div>
              )}
